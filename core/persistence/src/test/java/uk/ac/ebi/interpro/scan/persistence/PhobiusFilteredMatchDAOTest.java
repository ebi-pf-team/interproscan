...
public class PhobiusFilteredMatchDAOTest {
    ...
    public void phobiusMatchStoreTest(){ // Build some proteins and persist them.
        ...
        assertEquals(new Long(SEQUENCES.length), proteinDAO.count());
    }

    private List<Protein> insertProteinsInTransaction(List<Protein> proteins){
    ...
    private void insertMatchesInTransaction(List<Protein> proteins){
        ...
        assertEquals(proteins.size(), phobiusProteins.size());
        assertEquals(proteins.size(), PHOBIUS_FEATURE_LINES.length);
