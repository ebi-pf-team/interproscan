 ...
 * @since 1.0
 */
public class TMHMMFilteredMatchKVDAOImpl extends FilteredMatchKVDAOImpl<TMHMMMatch, RawMatch>
       implements TMHMMFilteredMatchKVDAO  {

    private static final Logger LOGGER = Logger.getLogger(TMHMMFilteredMatchKVDAOImpl.class.getName());

    private final SignatureLibraryRelease signatureLibraryRelease;

    public TMHMMFilteredMatchKVDAOImpl(SignatureLibraryRelease signatureLibraryRelease) {
    ...
    public void persist(Set<TMHMMProtein> proteins) {
        ...
        Map<String, HashSet<TMHMMMatch>> keyToMatchMap = new HashMap<>();
        for (TMHMMProtein protein : proteins) {
            ...
            for (TMHMMMatch match : protein.getMatches()) {
                ...
                String key =  protein.getProteinIdentifier()  + SignatureLibrary.TMHMM.getName();
                byte[] byteKey = SerializationUtils.serialize(key);
                byte[] byteMatch = SerializationUtils.serialize(match);
                HashSet<TMHMMMatch> matchesForThisKey = keyToMatchMap.get(key);
        ...
        Utilities.verboseLog("Time taken to process " + proteins.size() + " complete TMHMM raw protein-Matches and persist to kvStore : "
        ...
        while (it.hasNext()) {
            ...
            byte[] byteKey = SerializationUtils.serialize(key);
            byte[] byteMatch = SerializationUtils.serialize((HashSet<TMHMMMatch>) pair.getValue());
        ...
        if (proteins.size() > 0) {
            addSignatureLibraryName(SignatureLibrary.TMHMM.getName());
        }
    }
