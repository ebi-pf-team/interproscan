 ...
 * @since 1.0
 */
public class PhobiusFilteredMatchKVDAOImpl extends FilteredMatchKVDAOImpl<PhobiusMatch, RawMatch>
        implements SimpleProteinFilteredMatchKVDAO<PhobiusProtein, PhobiusMatch> {

    private static final Logger LOGGER = Logger.getLogger(PhobiusFilteredMatchKVDAOImpl.class.getName());
     ...
     */
    public PhobiusFilteredMatchKVDAOImpl(String phobiusVersion) {
    ...
    public void persist(Set<PhobiusProtein> phobiusProteins) {
        ...
        for (PhobiusProtein phobiusProtein : phobiusProteins) {
            ...
            HashSet<PhobiusMatch> proteinMatches = new HashSet<>();
            ...
            //persist the matches
            String key = Long.toString(persistentProtein.getId()) + SignatureLibrary.PHOBIUS.getName();
            byte[] byteKey = SerializationUtils.serialize(key);
            byte[] byteMatches = SerializationUtils.serialize(proteinMatches);
            //Utilities.verboseLog("To Persist key: " + key + " matches: " + proteinMatches.toString());
            persist(byteKey, byteMatches);
        }
        if (phobiusProteins.size() > 0) {
            //Utilities.verboseLog("addSignatureLibraryName : " + SignatureLibrary.PHOBIUS.getName());
            addSignatureLibraryName(SignatureLibrary.PHOBIUS.getName());
        }
    }
