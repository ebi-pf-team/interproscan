 ...
 */
abstract class Hmmer3FilteredMatchAndSiteKVDAO<T extends Hmmer3RawMatch, E extends Hmmer3RawSite>
        extends FilteredMatchAndSiteKVDAOImpl<T, Hmmer3MatchWithSites, E, Hmmer3MatchWithSites.Hmmer3LocationWithSites.Hmmer3Site> {

    private static final Logger LOGGER = Logger.getLogger(Hmmer3FilteredMatchAndSiteKVDAO.class.getName());

    public Hmmer3FilteredMatchAndSiteKVDAO() {
    ...
    public void persist(Collection<RawProtein<T>> filteredProteins, Collection<E> rawSites, final Map<String, Signature> modelAccessionToSignatureMap, final Map<String, Protein> proteinIdToProteinMap) {
        ...
        for (RawProtein<T> rp : filteredProteins) {
            ...
            if (signatureLibraryName == null) {
                if (rp.getMatches().size() > 0) {
                    Hmmer3RawMatch repRawMatch = new ArrayList<>(rp.getMatches()).get(0);
                    signatureLibraryName = repRawMatch.getSignatureLibrary().getName();
                }
            }
            ...
            byte[] byteMatches = SerializationUtils.serialize((HashSet<Hmmer3MatchWithSites>) filteredMatches);
