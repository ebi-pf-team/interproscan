...
class MobiDBFilteredMatchDAO extends FilteredMatchDAOImpl<MobiDBRawMatch, MobiDBMatch> {
    ...
    @Override
    @Transactional
    protected void persist(Collection<RawProtein<MobiDBRawMatch>> filteredProteins, Map<String, Signature> modelAccessionToSignatureMap, Map<String, Protein> proteinIdToProteinMap) {
...
//        Utilities.verboseLog("retrievedSignatures size: " + retrievedSignatures.size());
    ...
    private SignatureLibraryRelease loadMobiDBRelease() {
        ...
        if (releaseList.size() == 1 && releaseList.get(0) != null) {
            ...
            release = new SignatureLibraryRelease(SignatureLibrary.COILS, mobidbReleaseVersion);
