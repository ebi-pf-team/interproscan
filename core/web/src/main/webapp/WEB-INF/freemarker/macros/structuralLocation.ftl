...
<#macro structuralLocation id protein location structuralMatchData databaseMetadata>
...
<@locationMacro.location id="match-location-"+id protein=protein titlePrefix=title location=location colourClass=databaseName/>
...
<div id="match-popup-${id}" style="display: none;">
    ...
    <div class="popup_topl"><span class="${databaseName} caption_puce"></span>${location.start} - ${location.end}</div>
     <div class="popup_botl"><acronym title="${databaseMetadata.description}">${databaseMetadata.sourceName}</acronym><br/>
     ${links}<br/>
