...
my  $hmmNames = &getHmmNames("$library/names.tab") if ($namesFlag);

my $allScores = {};

if (! $usehmmsearch){
    my $hmmModel = "$library/panther.hmm";
	#$allScores = &runHmmscan($inFile,$hmmModel,$allScores);
	$allScores = &parseHmmscan($inFile,$hmmModel,$allScores);
} else{
   print STDERR "RUNNING parseHmmsearch\n";
    my $hmmModel = "$library/panther.hmm";
...
foreach my $seqId (keys %$allScores) {
    if ($allHits){
	    foreach my $eval (keys %{$allScores->{$seqId}}){
		    next if ($eval > $hmmEvalCut);
			foreach my $score (keys %{$allScores->{$seqId}{$eval}}){
			    foreach my $hmm (keys %{$allScores->{$seqId}{$eval}{$score}}){
				    my $seqRange;
					for my $range (keys %{$allScores->{$seqId}{$eval}{$score}{$hmm}}){
					    $seqRange = $seqRange.','.$range;
					}
					$seqRange =~ s/^\,//s;
					&printRes($seqId,$hmm,$eval,$score,$seqRange,$hmmNames->{$hmm});
				}
			}
		}
	}elsif ($pantherBestHit){
	    my ($best_hmms, $bestEval, $bestScore) = &getBestHit($allScores,$seqId,$hmmEvalCut);
		#my @best_hmms = @($best_hmms);
		next unless (@{$best_hmms});
		foreach my $hmm (@{$best_hmms}){
		    my $seqRange;
			for my $range (keys %{$allScores->{$seqId}{$bestEval}{$bestScore}{$hmm}}){
				$seqRange = $seqRange.','.$range;
			}
			$seqRange =~ s/^\,//s;
			#print "$hmm\t$bestEval\t$seqRange\n";
			&printRes($seqId,$hmm,$bestEval,$bestScore,$seqRange,$hmmNames->{$hmm});
		}
	}elsif ($iprScanBestHit) {  
		my ($best_hmms, $bestEval, $bestScore) = &getBestHit($allScores,$seqId,$hmmEvalCut);
		next unless (@{$best_hmms});
		foreach my $hmm (@{$best_hmms}){
			...
			$seqRange =~ s/^\,//s;
			&printRes($seqId,$hmm,$bestEval,$bestScore,$seqRange,$hmmNames->{$hmm}, 1);
			my ($fam,$sf) = split(/:/,$hmm);
			if ($sf) {
			  #if doing ipr scan and have SF hit, get fam info (if fam meet threshold)
			  #get sf score info, and use sf seq range
			  #&printRes($seqId,$fam,$allScores->{$seqId}{$hmm}{'eval'},$allScores->{$seqId}{$hmm}{'score'},$allScores->{$seqId}{$hmm}{'seqRange'},$hmmNames->{$fam},1);
		      &printRes($seqId,$fam,$bestEval,$bestScore,$seqRange,$hmmNames->{$fam},1);
			}
		}
    }
}
...
sub printRes {
  ...
  if ($splitRanges) {
	...
	my @posis;
    foreach my $range (sort @ranges) {
      my @positions = split(/-/, $range);
	  foreach my $pos (@positions){
	    push (@posis, $pos);
	  }
    }
	$seqRange = "$posis[0]-$posis[-1]";
...
sub getBestHit {
    ...
    return (\@bestHmm, $bestEval, $bestScore);
}
