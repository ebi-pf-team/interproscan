...
my $curWorkDir = "${tmpdirPath}/${jobHostname}";

unless(-e $curWorkDir or mkdir $curWorkDir) {
        die "Unable to create $curWorkDir";
}

my $count;
opendir(my $dh, $curWorkDir) or die "opendir($curWorkDir): $!";
...
if ($count > 2000){
 $curWorkDir = "${curWorkDir}_1";
 unless(-e $curWorkDir or mkdir $curWorkDir) {
        die "Unable to create $curWorkDir";
 }
}

my $infiledigest = md5_hex($inFile);
my $i5PantherTmpDir = "${curWorkDir}/${infiledigest}/";

unless(-e $i5PantherTmpDir or mkdir $i5PantherTmpDir) {
        die "Unable to create $i5PantherTmpDir";
}

$tmpdirPath = $i5PantherTmpDir;
