...
sub init {
  ...
  #unparsed results
  $self->{unparsedSeqStart} = $seqStart;
  $self->{unparsedSeqEnd} = $seqEnd;
  $self->{unparsedHmmStart} = $hmmStart;
  $self->{unparsedHmmEnd} = $hmmEnd;

  #calculate starts and ends based on aliged regions (ex: for interpro)
  $self->_setCalculatedStartsEndsRanges($seqStart,$hmmStart);
}
