...
public class RunSerialWorker {
    ...
    public static void main(String[] args) {
        ...
        AbstractApplicationContext ctx = new ClassPathXmlApplicationContext(new String []{"spring-config-serial-worker.xml"});
        ctx.registerShutdownHook();
        Worker worker = (Worker) ctx.getBean("serialWorker");
        worker.start();
        ctx.close();
    }
}